    ...
    fn get_step_err(
            ...
            && next_pc != 0
        {
            ...
            if step.op == OpcodeId::CREATE2 {
                ...
                //      return Some(ExecError::AddressCollision);
                // }
            }
...
mod tracer_tests {
    ...
    // Geth Errors ignored
    //
    // These errors happen in a CALL, CALLCODE, DELEGATECALL or STATICCALL, and are used internally
    // but not propagated in geth to the scope where the tracer is used.
    ...
    fn tracer_err_depth() {
        ...
        assert_eq!(check_err_depth(step, next_step), true);
    ...
    fn tracer_err_insufficient_balance() {
        ...
        // TODO: Uncomment once get_step_err correctly detects InsufficientBalance
        // let mut builder = CircuitInputBuilderTx::new(&block, &step);
        // assert_eq!(
        //     builder.state_ref().get_step_err(&step, next_step),
    ...
    fn tracer_err_address_collision() {
        ...
        let (index, step) = block
            ...
            .unwrap();
        let next_step = block.geth_trace.struct_logs.get(index + 1);
        assert_eq!(check_err_address_collision(step, next_step), true);

        // TODO: Uncomment once get_step_err correctly detects AddressCollision
        // let mut builder = CircuitInputBuilderTx::new(&block, &step);
        // assert_eq!(
        //     builder.state_ref().get_step_err(&step, next_step),
        //     Some(ExecError::AddressCollision)
        // );
    }
    ...
    fn tracer_err_code_store_out_of_gas() {
        ...
        let (index, step) = block
            ...
            .unwrap();
        let next_step = block.geth_trace.struct_logs.get(index + 1);
        assert_eq!(check_err_code_store_out_of_gas(step, next_step), true);
    ...
    //
    // These errors are specific to some opcodes and due to the way the tracing works, they are
    // never captured, because the trace is made before the step is executed, so when these errors
    // happen, the trace step contains error = null.
    ...
    fn tracer_err_return_data_out_of_bounds() {
        ...
        let block =
            mock::BlockData::new_single_tx_trace_code_2(&code_a, &code_b)
                .unwrap();
        // println!("DBG {:#?}", block.geth_trace.struct_logs);
